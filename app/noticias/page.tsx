...
export default async function NoticiasPage() {
  ...
  return <main className="site-shell"><SiteHeader />
    ...
    <section className="section news-section"><div className="container"><div className="news-toolbar"><div><span className="news-live"><i /> Atualização automática</span><p>Conteúdo renovado aproximadamente a cada 30 minutos, diretamente de diferentes publicações.</p></div><span className="news-source-count">{newsSources.length} fontes internacionais</span></div>
      ...
      <div className="news-grid">{items.map((item, index) => <article className={index === 0 ? "news-featured" : ""} key={`${item.link}-${index}`}><a className={`news-cover ${item.image ? "has-image" : ""}`} href={item.link} target="_blank" rel="noreferrer" style={item.image ? { backgroundImage: `linear-gradient(180deg,transparent 35%,rgba(3,28,45,.55)),url(${JSON.stringify(item.image).slice(1,-1)})` } : undefined} aria-label={`Abrir: ${item.title}`}><span>{item.source}</span></a><div className="news-card-body"><div className="news-meta"><span>{item.category}</span><time>{item.date && !Number.isNaN(new Date(item.date).getTime()) ? new Intl.DateTimeFormat("pt-BR", { day:"2-digit", month:"short", year:"numeric" }).format(new Date(item.date)) : "Seleção atual"}</time></div><h2>{item.title}</h2><p>{item.summary || "Leia os detalhes e a análise completa na publicação original."}</p><div className="news-card-footer"><small>Fonte: {item.source}</small><a href={item.link} target="_blank" rel="noreferrer">{news.length ? "Ler notícia completa" : "Visitar publicação"} <ArrowIcon diagonal /></a></div></div></article>)}</div>
    </div></section>

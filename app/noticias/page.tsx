...
export default async function NoticiasPage() {
  ...
  return <main className="site-shell"><SiteHeader />
    ...
    <section className="section news-section"><div className="container"><div className="news-toolbar"><div><span className="news-live"><i /> Atualização automática</span><p>Conteúdo renovado aproximadamente a cada 30 minutos.</p></div><a href="https://news.google.com/" target="_blank" rel="noreferrer">Ver central de notícias ↗</a></div>
      {news.length ? <div className="news-grid">{news.map((item, index) => <article className={index === 0 ? "news-featured" : ""} key={`${item.link}-${index}`}><div className="news-meta"><span>{item.category}</span><time>{item.date ? new Intl.DateTimeFormat("pt-BR", { day:"2-digit", month:"short", year:"numeric" }).format(new Date(item.date)) : "Atual"}</time></div><h2>{item.title}</h2><p>Fonte: {item.source}</p><a href={item.link} target="_blank" rel="noreferrer">Ler notícia completa <span>↗</span></a></article>)}</div> : <div className="news-fallback"><span>◎</span><h2>Atualizando o radar mundial</h2><p>A fonte está temporariamente indisponível. Tente novamente em alguns minutos ou abra a central de notícias.</p><a className="button" href="https://news.google.com/" target="_blank" rel="noreferrer">Acessar notícias <span>↗</span></a></div>}
    </div></section>

...
import { approveProposal, confirmPixAndPublish, createProposal, signOut } from "./actions";

export type Role = "client" | "team";
type View = "inicio" | "projetos" | "entregas" | "suporte" | "agenda";
export type CommerceProject = { id:string; code:string; name:string; project_type:string; status:string; progress:number; created_at:string; previewUrl:string | null; finalUrl:string | null; proposal:null | { id:string; amountCents:number; status:string; provider:string | null; description:string } };
...
function ProposalAction({ role, project }: { role:Role; project:CommerceProject }) {
  ...
  if (role === "team") return proposal.status === "pending" && proposal.provider === "inter_manual" ? <form action={confirmPixAndPublish}><input type="hidden" name="paymentId" value={proposal.id}/><button className="hub-primary">{project.project_type === "Sistema personalizado" ? "Confirmar Pix" : "Confirmar Pix e publicar"} <ArrowIcon /></button></form> : <span className="proposal-state">{proposal.status === "proposal" ? "Aguardando aceite" : proposal.status === "confirmed" ? "Pagamento confirmado" : "Aguardando pagamento"}</span>;
  ...
  if (proposal.status === "pending") return <div className="pix-action">{pixCode ? <><label>{manualPix ? "Chave Pix aleatória" : "Pix Copia e Cola"}<textarea readOnly value={pixCode} rows={manualPix ? 2 : 4}/></label><button className="hub-primary" onClick={() => void navigator.clipboard.writeText(pixCode)}>Copiar {manualPix ? "chave Pix" : "código Pix"}</button><small>{manualPix ? `Transfira exatamente ${money(proposal.amountCents)} e envie o comprovante à LESystems. A confirmação será realizada pela equipe.` : "A confirmação aparecerá automaticamente no Hub."}</small></> : <><button className="hub-primary" onClick={async () => { setPaymentError(""); const response = await fetch("/api/payments/checkout", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ paymentId:proposal.id }) }); const data = await response.json(); if (data.pixCode) { setPixCode(data.pixCode); setManualPix(Boolean(data.manual)); } else setPaymentError(data.error || "Pagamento indisponível."); }}>Gerar Pix Banco Inter <ArrowIcon diagonal /></button>{paymentError && <small className="payment-error">{paymentError}</small>}</>}</div>;
  return <span className="proposal-state paid">Pagamento confirmado</span>;
}

function money(cents:number){return new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(cents/100)}
function statusLabel(status:string){return ({new_request:"Nova solicitação",brief_received:"Briefing recebido",under_analysis:"Em análise",preview_released:"Protótipo disponível",awaiting_approval:"Aguardando aprovação",awaiting_payment:"Aguardando pagamento",payment_confirmed:"Pagamento confirmado",in_development:"Em desenvolvimento",published:"Publicado",delivered:"Entregue"} as Record<string,string>)[status] || status.replaceAll("_"," ")}

...
export default function ConteudosPage() {
  ...
  return <main className="site-shell"><SiteHeader />
    ...
    <section className="section social-section"><div className="container">
      <div className="social-channels">{channels.map((channel) => <article key={channel.name}><span>{channel.symbol}</span><div><h2>{channel.name}</h2><p>{channel.text}</p></div>{channel.href ? <a className="inline-action" href={channel.href} target="_blank" rel="noreferrer">Ver perfil oficial <ArrowIcon /></a> : <small>Use o QR abaixo para acessar</small>}</article>)}</div>
    ...
    <section className="section social-cta"><div className="container contact-card light-card"><div><p className="eyebrow"><span /> Continue a conversa</p><h2>Viu algo interessante?<br /><em>Fale com a equipe.</em></h2></div><Link className="button" href="/contato">Entrar em contato <ArrowIcon diagonal /></Link></div></section><SiteFooter /></main>;
}

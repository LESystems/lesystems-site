...
function ProposalAction({ role, project }: { role:Role; project:CommerceProject }) {
  ...
  if (proposal.status === "pending") return <div className="pix-action">{pixCode ? <><label>Pix Copia e Cola<textarea readOnly value={pixCode} rows={4}/></label><button className="hub-primary" onClick={() => void navigator.clipboard.writeText(pixCode)}>Copiar código Pix</button><small>A confirmação aparecerá automaticamente no Hub.</small></> : <><button className="hub-primary" onClick={async () => { setPaymentError(""); const response = await fetch("/api/payments/checkout", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ paymentId:proposal.id }) }); const data = await response.json(); if (data.pixCode) setPixCode(data.pixCode); else setPaymentError(data.error || "Pagamento indisponível."); }}>Gerar Pix Banco Inter <ArrowIcon diagonal /></button>{paymentError && <small className="payment-error">{paymentError}</small>}</>}</div>;

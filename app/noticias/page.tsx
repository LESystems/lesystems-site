...
export default async function NoticiasPage() {
  ...
  return <main className="site-shell"><SiteHeader />
    ...
    <section className="section news-section"><div className="container"><div className="news-toolbar"><div><span className="news-live"><i /> Atualização automática</span><p>Conteúdo renovado aproximadamente a cada 30 minutos.</p></div><a className="inline-action" href="https://news.google.com/" target="_blank" rel="noreferrer">Ver central de notícias <ArrowIcon diagonal /></a></div>
      <div className="news-grid">{items.map((item, index) => <article className={index === 0 ? "news-featured" : ""} key={`${item.link}-${index}`}><div className="news-meta"><span>{item.category}</span><time>{item.date ? new Intl.DateTimeFormat("pt-BR", { day:"2-digit", month:"short", year:"numeric" }).format(new Date(item.date)) : "Seleção atual"}</time></div><h2>{item.title}</h2><p>Fonte: {item.source}</p><a href={item.link} target="_blank" rel="noreferrer">{news.length ? "Ler notícia completa" : "Ver notícias desta categoria"} <ArrowIcon diagonal /></a></article>)}</div>
    </div></section>

...
export default function EmpresaPage() {
  return <main className="site-shell"><SiteHeader />
    <section className="company-hero"><Image src="/company-capabilities.webp" alt="Representação de sistemas, dados, automação e colaboração" fill priority sizes="100vw" /><div className="company-veil" /><div className="container company-hero-copy"><p className="eyebrow"><span /> Conheça a LESystems</p><h1>Tecnologia próxima,<br /><em>clara e útil.</em></h1><p>Unimos estratégia, engenharia de software e acompanhamento humano para transformar necessidades reais em soluções digitais que fazem sentido.</p><Link className="button" href="/contato">Apresente seu desafio <span>↗</span></Link></div></section>
    ...
    <section className="section competencies-section"><div className="container"><div className="section-heading"><div><p className="eyebrow"><span /> Competências</p><h2>Conhecimento conectado<br /><em>ao seu resultado.</em></h2></div><p>Competências complementares para planejar, criar, integrar e melhorar toda a experiência digital.</p></div><div className="competency-grid">{capabilities.map(([number,title,text]) => <article key={number}><span>{number}</span><div><h3>{title}</h3><p>{text}</p></div></article>)}</div></div></section>
    <section className="section company-principles"><div className="container"><p className="eyebrow"><span /> Como trabalhamos</p><div className="principle-grid"><article><strong>Clareza</strong><p>Você entende as escolhas, etapas e prioridades do projeto.</p></article><article><strong>Intenção</strong><p>Cada recurso existe para resolver uma necessidade real.</p></article><article><strong>Parceria</strong><p>Tecnologia e atendimento humano continuam próximos.</p></article></div><div className="center-action"><Link href="/servicos" className="button">Conhecer todas as soluções <span>→</span></Link></div></div></section><SiteFooter />
  </main>;
}

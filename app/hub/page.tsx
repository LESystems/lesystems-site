...
export default async function HubPage() {
  ...
  const { data: projects } = await supabase.from("projects").select("id,public_id,name,project_type,status,progress,created_at,final_url").order("created_at", { ascending: false });
  const ids = (projects || []).map(project => project.id);
  const { data: payments } = ids.length ? await supabase.from("payments").select("id,project_id,version_id,amount_cents,status,provider,provider_reference").in("project_id", ids).order("created_at", { ascending: false }) : { data: [] };
  const { data: previews } = ids.length ? await supabase.from("project_previews").select("project_id,url").in("project_id", ids).order("created_at", { ascending:false }) : { data: [] };
  ...
  const commerceProjects: CommerceProject[] = (projects || []).map(project => {
    ...
    return { ...project, code: `LE-${String(project.public_id).padStart(6, "0")}`, finalUrl:project.final_url, previewUrl:(previews || []).find(item => item.project_id === project.id)?.url || null, proposal: payment ? { id: payment.id, amountCents: payment.amount_cents || 0, status: payment.status, provider:payment.provider, description: snapshot?.description || "Proposta comercial LESystems" } : null };
  });

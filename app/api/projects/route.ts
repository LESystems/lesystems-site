...
import { quoteProduct } from "@/app/product-catalog";
...
const briefSchema = z.object({
  ...
  packageId: z.string().trim().max(40).optional().default(""), addons: z.string().trim().max(500).optional().default(""),
});
...
export async function POST(request: Request) {
  try {
    ...
    const selectedQuote = quoteProduct(input.packageId, input.addons.split(",").filter(Boolean));
    const projectType = selectedQuote ? selectedQuote.product.id === "landing" ? "Landing page" : selectedQuote.product.id === "portal" ? "Portal simples" : "Site institucional" : input.projectType;
    ...
    const snapshot = { type: projectType === "Sistema personalizado" ? "system-prototype" : "website-draft", company: input.companyName, segment: input.segment, description: input.companyDescription, audience: input.audience, objective: input.objective, features, colors: input.colors, style: input.style, contactName: input.contactName };
    ...
    if (selectedQuote) {
      const description = `${selectedQuote.product.name}. Inclusos: ${selectedQuote.product.includes.join(", ")}${selectedQuote.addons.length ? `. Adicionais: ${selectedQuote.addons.map(item => item.name).join(", ")}` : ""}. Prazo estimado: ${selectedQuote.product.delivery}.`;
      const { data:proposalVersion } = await supabase.from("project_versions").insert({ project_id:project.id, version:`proposta-automatica-${Date.now()}`, snapshot:{ type:"commercial_proposal", description, package_id:selectedQuote.product.id, addons:selectedQuote.addons.map(item=>item.id) } }).select("id").single();
      if (proposalVersion) {
        await supabase.from("payments").insert({ project_id:project.id, version_id:proposalVersion.id, amount_cents:selectedQuote.totalCents, status:"proposal" });
        await supabase.from("projects").update({ status:"awaiting_approval" }).eq("id", project.id);
      }
    }
